...
async fn check(infura_key: &str) -> web3::contract::Result<U256> {
    let transport = web3::transports::WebSocket::new(
        format!("wss://mainnet.infura.io/ws/v3/{}", infura_key).as_str(),
    )
    .await?;
...
async fn supply() -> impl Responder {
    match env::var("INFURA_KEY") {
        ...
        Ok(infura_key) => match check(&infura_key).await {
...
async fn main() -> std::io::Result<()> {
    dotenv().ok();
    HttpServer::new(|| App::new().service(supply).service(total).service(index))
        .bind("127.0.0.1:8080")?
        .run()
        .await
}
